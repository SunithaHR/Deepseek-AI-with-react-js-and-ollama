...
// Assuming `chat` is an API function that uses a POST request to fetch data from a backend API
  ...
  try {
    const response = await fetch("http://127.0.0.1:11434/api/chat", {
      ...
      body: JSON.stringify({
        ...
        streaming: true,
        options: { temperature: 0.1, repeat_penalty: 1.2, numa: true },
        messages,
      }),
    });
...
function AIChat() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const stream = await chat([{ role: "system", content: premise }, ...newMessages]);
        ...
        for (const line of lines) {
          ...
          try {
            ...
            setMessages((prev) => {
              ...
              return lastMsg?.role === "assistant" ? [...prev.slice(0, -1), { role: "assistant", content: assistantResponse }] : [...prev, { role: "assistant", content: assistantResponse }];
            });
          } catch (error) {
